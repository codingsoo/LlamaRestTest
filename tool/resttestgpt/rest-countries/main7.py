...
if __name__ == '__main__':
    openapi_spec_file = "../../../specs/openapi/rest-countries.yaml"
    restgpt_file = "../../../restgpt/rest-countries.json"
    openai_api_key = "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"
    ...
    for op_id in operations:
        ...
        for param in operations[op_id]['parameters']:
            ...
            if param in restgpt[path][method]:
                provided_object = {}
                generated_object = {}
                for rule in restgpt[path][method][param]:
                    if param == "body" and restgpt[path][method][param][rule]:
                        if rule == "":
                            gpt_spec[op_id]["parameters"][param]["examples"] = restgpt[path][method][param][""]["examples"]["provided"]
                            gpt_spec[op_id]["parameters"][param]["generated"] = restgpt[path][method][param][""]["examples"]["generated"]
                        else:
                            provided_object[rule] = restgpt[path][method][param][rule]["examples"]["provided"]
                            generated_object[rule] = restgpt[path][method][param][rule]["examples"]["generated"]
                    elif rule == "examples":
                        gpt_spec[op_id]["parameters"][param]["examples"] = restgpt[path][method][param][rule]["provided"]
                        gpt_spec[op_id]["parameters"][param]["generated"] = restgpt[path][method][param][rule]["generated"]
                    elif rule == "x-dependencies":
                        for IPD in restgpt[path][method][param][rule]:
                            count = 0
                            for key in operations[op_id]['parameters'].keys():
                                if key in IPD:
                                    count += 1
                            if count == 2:
                                if IPD not in gpt_spec[op_id]["parameters"][param]["IPD"]:
                                    gpt_spec[op_id]["parameters"][param]["IPD"].append(IPD)
                    elif rule == "min":
                        gpt_spec[op_id]["parameters"][param]["minimum"] = restgpt[path][method][param][rule]
                    elif rule == "max":
                        gpt_spec[op_id]["parameters"][param]["maximum"] = restgpt[path][method][param][rule]
                    elif rule == "minLength":
                        gpt_spec[op_id]["parameters"][param]["minimum"] = restgpt[path][method][param][rule]
                    elif rule == "maxLength":
                        gpt_spec[op_id]["parameters"][param]["maximum"] = restgpt[path][method][param][rule]
                if provided_object:
                    gpt_spec[op_id]["parameters"][param]["examples"] = provided_object
                if generated_object:
                    gpt_spec[op_id]["parameters"][param]["examples"] = generated_object
    for sequence in test_sequence:
